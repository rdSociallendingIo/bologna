...
console.info("process.env.LOG4JS_CONFIG <== ", process.env.LOG4JS_CONFIG);
...
global.loggerEth = log4js.getLogger('bologna');
...
loggerEth.info("WS_SECRET <== ", WS_SECRET);
...
{
	loggerEth.info("Nodes <== ", Nodes);
	if(err !== null)
	{
		loggerEth.error('COL', 'CHR', 'Charts error:', err);
	}
	else
	{
		client.write({
...
{
	loggerEth.info("connection <== ", spark);
	loggerEth.debug('API', 'CON', 'Open:', spark.address.ip);

	spark.on('hello', function (data)
	{
		loggerEth.debug('API', 'CON', 'Hello', data['id']);

		if( _.isUndefined(data.secret) || WS_SECRET.indexOf(data.secret) === -1 || banned.indexOf(spark.address.ip) >= 0 )
		{
			spark.end(undefined, { reconnect: false });
			loggerEth.error('API', 'CON', 'Closed - wrong auth', data);
		...
		if( !_.isUndefined(data.id) && !_.isUndefined(data.info) )
		{
			...
			Nodes.add( data, function (err, info)
			{
				if(err !== null)
				{
					loggerEth.error('API', 'CON', 'Connection error:', err);
					return false;
				}
				...
				if(info !== null)
				{
					...
					loggerEth.debug('API', 'CON', 'Connected', data.id);
	...
	spark.on('update', function (data)
	{
		if( !_.isUndefined(data.id) && !_.isUndefined(data.stats) )
		{
			Nodes.update(data.id, data.stats, function (err, stats)
			{
				if(err !== null)
				{
					loggerEth.error('API', 'UPD', 'Update error:', err);
				}
				else
				{
					if(stats !== null)
					{
						...
						loggerEth.debug('API', 'UPD', 'Update from:', data.id, 'for:', stats);
		...
		{
			loggerEth.error('API', 'UPD', 'Update error:', data);
		}
	});
	...
	spark.on('block', function (data)
	{
		if( !_.isUndefined(data.id) && !_.isUndefined(data.block) )
		{
			Nodes.addBlock(data.id, data.block, function (err, stats)
			{
				if(err !== null)
				{
					loggerEth.error('API', 'BLK', 'Block error:', err);
				}
				else
				{
					if(stats !== null)
					{
						...
						loggerEth.debug('API', 'BLK', 'Block:', data.block['number'], 'from:', data.id);
		...
		{
			loggerEth.error('API', 'BLK', 'Block error:', data);
		}
	});
	...
	spark.on('pending', function (data)
	{
		if( !_.isUndefined(data.id) && !_.isUndefined(data.stats) )
		{
			Nodes.updatePending(data.id, data.stats, function (err, stats) {
				if(err !== null)
				{
					loggerEth.error('API', 'TXS', 'Pending error:', err);
				}
				...
				if(stats !== null)
				{
					...
					loggerEth.debug('API', 'TXS', 'Pending:', data.stats['pending'], 'from:', data.id);
				}
			});
		}
		else
		{
			loggerEth.error('API', 'TXS', 'Pending error:', data);
		}
	});
	...
	spark.on('stats', function (data)
	{
		if( !_.isUndefined(data.id) && !_.isUndefined(data.stats) )
		{
			...
			Nodes.updateStats(data.id, data.stats, function (err, stats)
			{
				if(err !== null)
				{
					loggerEth.error('API', 'STA', 'Stats error:', err);
				}
				else
				{
					if(stats !== null)
					{
						...
						loggerEth.debug('API', 'STA', 'Stats from:', data.id);
					}
				}
			});
		}
		...
		{
			loggerEth.error('API', 'STA', 'Stats error:', data);
		}
	});
	...
	{
		loggerEth.debug('API', 'HIS', 'Got history from:', data.id);

		var time = chalk.reset.cyan((new Date()).toJSON()) + " ";
		loggerEth.debug(time, 'COL', 'CHR', 'Got charts in');
		...
		Nodes.addHistory(data.id, data.history, function (err, history)
		{
			...
			if(err !== null)
			{
				loggerEth.error('COL', 'CHR', 'History error:', err);
			}
			else
			{
				client.write({
	...
	spark.on('node-ping', function (data)
	{
		...
		loggerEth.debug('API', 'PIN', 'Ping from:', data['id']);
	});
	...
	spark.on('latency', function (data)
	{
		if( !_.isUndefined(data.id) )
		{
			Nodes.updateLatency(data.id, data.latency, function (err, latency)
			{
				if(err !== null)
				{
					loggerEth.error('API', 'PIN', 'Latency error:', err);
				}
				...
				if(latency !== null)
				{
					...
					loggerEth.debug('API', 'PIN', 'Latency:', latency, 'from:', data.id);
				}
			});
			...
			if( Nodes.requiresUpdate(data.id) )
			{
				...
				loggerEth.debug('API', 'HIS', 'Asked:', data.id, 'for history:', range.min, '-', range.max);
	...
	spark.on('end', function (data)
	{
		Nodes.inactive(spark.id, function (err, stats)
		{
			if(err !== null)
			{
				loggerEth.error('API', 'CON', 'Connection end error:', err);
			}
			else
			{
				client.write({
				...
				loggerEth.warn('API', 'CON', 'Connection with:', spark.id, 'ended:', data);
			}
		});
	});
});
...
loggerEth.debug("server_port >>> ", server_port);
